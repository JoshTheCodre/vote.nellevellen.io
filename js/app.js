...
function renderPositionCards(positionsData, votedPositions, grid) {
  positionsData.forEach(function(p) {
    ...
    const html = `
      <div class="backdrop-blur-xl bg-white/10 border border-white/25 rounded-2xl p-6 hover:bg-white/15 transition-all duration-300 hover:shadow-xl hover:border-white/40 transform hover:-translate-y-1" data-position-id="${p.id}">
        ${voted ? '<div class="mb-4 inline-flex items-center gap-2 bg-emerald-400/20 border border-emerald-400/50 px-3 py-1 rounded-full"><svg class="w-4 h-4 text-emerald-300" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"></path></svg><span class="text-emerald-300 text-sm font-semibold">Voted</span></div>' : ''}
        <div class="mb-4">
          <div class="h-1.5 w-12 bg-gradient-to-r from-green-400 to-emerald-400 rounded-full mb-4"></div>
          <h3 class="text-lg font-bold text-white mb-1">${p.name}</h3>
          <p class="text-sm text-white/70">Select your preferred candidate</p>
        </div>
        <select class="w-full px-4 py-3 mb-4 bg-white/10 border border-white/20 rounded-lg text-white placeholder-white/50 focus:outline-none focus:border-green-400/50 focus:ring-2 focus:ring-green-400/20 transition duration-200 appearance-none cursor-pointer" data-position-id="${p.id}" ${voted ? 'disabled' : ''}>
          <option value="" selected hidden class="bg-gray-900 text-white">Choose candidate...</option>
        </select>
        <button class="w-full bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-600 hover:to-emerald-700 disabled:from-gray-500 disabled:to-gray-600 disabled:opacity-50 disabled:cursor-not-allowed text-white font-semibold py-2.5 px-4 rounded-lg transition-all duration-200 active:scale-95 shadow-lg hover:shadow-xl" data-position-id="${p.id}" ${voted ? 'disabled' : ''}>Vote Now</button>
      </div>
    `;
    grid.append(html);
